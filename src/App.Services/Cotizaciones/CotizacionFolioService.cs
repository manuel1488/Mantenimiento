...
public class CotizacionFolioService : ICotizacionFolioService
{
    private readonly IDbContextFactory<ApplicationDbContext> _contextFactory;
    private readonly IDateTime _dateTimeService;
    private readonly ICompanySettingsService _companySettingsService;

    public CotizacionFolioService(
    ...
    public async Task<(int Anio, int Numero)> GenerarSiguienteFolioAsync(CancellationToken cancellationToken = default)
    {
        ...
        var maxNumero = await context.Cotizaciones
            .Where(c => c.FolioAnio == anio)
            .Select(c => (int?)c.FolioNumero)
